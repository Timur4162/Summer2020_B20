...
public class Remove_Digits_Letters {
    public static void main(String[] args) {
        ...
        digits.addAll(list);
        digits.removeIf(p -> !Character.isDigit(p));// if the character is not digit => remove from LIST
        System.out.println("Digits: " + digits);


        ArrayList<Character> letters = new ArrayList<>();//  [a,b,c,d]
        letters.addAll(list);
        letters.removeIf(p -> !Character.isLetter(p)); // if the character is letter, then remove it from LIST
